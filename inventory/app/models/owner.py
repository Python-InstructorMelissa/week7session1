...
class Owner:
    ...
    def getListWithItems(cls, data):
        q = "SELECT * FROM owner LEFT JOIN owner_has_inventory ON owner_id = owner.id LEFT JOIN inventory ON inventory.id = inventory_id LEFT JOIN user ON user.id = owner.user_id WHERE owner.id = %(id)s;"
        r = connectToMySQL(cls.db_name).query_db(q, data)
        print("owners Lists with inventory: ", r)
        return r